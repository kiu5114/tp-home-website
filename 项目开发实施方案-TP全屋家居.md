...
- 轮播/亮点/系列/空间/单品/案例/新闻+分类/职位/投递/门店/关于页+历程/线索(+导出 Excel)/管理员/角色/权限字典/部门/菜单/岗位/字典/通知公告/操作日志/登录日志/在线用户/站点配置/上传/看板统计。
...
### 6.2 模块映射（PRD §6 → 页面，侧栏多级分组）

后台侧栏为多级分组结构（菜单级 RBAC，开发技术文档 §8.3）：
- **仪表盘**：运营看板（核心数据总览，ECharts 三折线：在线留言蓝/预约到店金/招聘投递绿，§3.7）。
- **内容管理**：首页配置（Banner/亮点/推荐产品/推荐案例）、关于我们内容（关于TP/品牌/历程/联系信息）、门店管理、站点配置、新闻管理（分类/文章）。
- **产品与案例**：产品管理（系列/空间分类/单品）、案例管理。
- **留言与招聘**：留言预约管理（Lead 列表/流转/导出 Excel）、招聘管理（职位/投递）。
- **系统管理**：用户管理、角色管理（含权限字典只读）、菜单管理、部门管理、岗位管理、字典管理、通知公告。
- **系统监控**：操作日志、登录日志、在线用户。
- **官网入口**：外链打开前台（新标签页）。

> 阶段二（2026-08-20）将系统管理由「四 Tab 合并页」拆分为用户/角色/部门独立页面，并新增菜单/岗位/字典/通知公告四个 CRUD 模块 + 登录日志/在线用户两个只读监控模块（后端新增 Menu/DictType/DictData/Notice/LoginLog/Post 六张表，见数据库设计文档 §4.22–§4.27）。
...
> **后台菜单重构（2026-08-20）**：✅ **侧栏多级分组**：将原有扁平菜单改为 仪表盘/内容管理/产品与案例/留言与招聘/系统管理/系统监控 六分组 + 官网外链（新标签页）；运营看板置于"仪表盘"分组下，新闻管理改名为"新闻动态"。✅ **系统管理拆分**：原"系统管理"四 Tab 合并页拆分为用户管理(/admins)、角色管理(/roles，内含权限字典只读)、部门管理(/departments) 三个独立页面。✅ **阶段二新模块**：后端新增 Menu/DictType/DictData/Notice/LoginLog/Post 六张表（models.py + seed 种子：菜单 26 条镜像侧栏、字典类型/数据、通知公告、岗位），并新增对应 CRUD 路由（菜单/岗位/字典/通知公告）+ 只读路由（登录日志/在线用户）；auth.py 登录成功/登出写入 LoginLog；seed 同步超级管理员权限码至全量。前端新增 MenusManage/PostsManage/DictsManage/NoticesManage/LoginLogs/OnlineUsers 六个页面（React.lazy 分包），全部通过 `tsc -b` 类型检查与生产构建。
